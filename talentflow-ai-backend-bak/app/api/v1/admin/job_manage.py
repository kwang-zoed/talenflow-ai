...
async def get_jobs(
    ...
    print(f"[GET /admin/jobs] 异步接口开始")
    
    jobs = await crud.read_jobs_async(db, skip=skip, limit=limit, keyword=keyword)
    
    print(f"[GET /admin/jobs] 异步接口完成: 耗时={time.time() - start_time:.3f}s, 职位数={len(jobs)}")
...
async def parse_job_document(
    ...
    raise HTTPException(
        status_code=410,
        detail="同步职位解析已废弃，请使用 POST /admin/jobs/parse/submit 与 GET /admin/jobs/parse/status/{task_id}",
    )
...
async def batch_parse_job_documents(
    ...
    raise HTTPException(
        status_code=410,
        detail="同步批量解析已废弃，请使用 POST /admin/jobs/parse/submit（is_batch=true）与 GET /admin/jobs/parse/status/{task_id}",
    )
...
async def submit_parse_task(
    ...
    print(f"[parse submit] 任务已提交: task_id={task.id}, file={filename}, is_batch={is_batch}")
    return {
        "task_id": task.id,
        "filename": filename,
        "is_batch": is_batch,
