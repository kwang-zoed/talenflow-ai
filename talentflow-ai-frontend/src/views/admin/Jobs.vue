...
import { submitParseTask, pollParseTaskResult } from '../../api/job';
...
const STORAGE_KEY_PARSE_TASK = 'talentflow_admin_parse_task'
const STORAGE_KEY_PARSE_RESULT = 'talentflow_admin_parse_result'
...
function getSavedParseTask() {
  try {
    ...
    console.warn('Failed to get task from localStorage:', e)
  }
  return null
}
...
function clearSavedParseTask() {
  try {
    localStorage.removeItem(STORAGE_KEY_PARSE_TASK)
  } catch (e) {
    console.warn('Failed to clear task from localStorage:', e)
  }
}
...
function getCompletedParseResult() {
  try {
    ...
    if (saved) {
      ...
      if (Date.now() - wrapper.timestamp < 600000) {
        return wrapper.data
      }
    }
  } catch (e) {}
...
function clearCompletedParseResult() {
  try {
    localStorage.removeItem(STORAGE_KEY_PARSE_RESULT)
  } catch (e) {}
}
...
const handleUploadParse = async ({ file, onSuccess, onError }) => {
  try {
    ...
    console.log('[Jobs.vue] 解析完成:', finalResult)
    ...
    console.error('[Jobs.vue] 解析失败:', err)
    clearSavedParseTask()
    ElMessage.error(err.message || '解析失败，请重试')
...
const handleBatchUploadParse = async ({ file, onSuccess, onError }) => {
  try {
    ...
    const submitRes = await submitParseTask(file, true)
    const taskId = submitRes.data?.task_id || submitRes.task_id
    const filename = submitRes.data?.filename || file.name
    console.log('[Jobs.vue] 批量解析任务已提交 task_id:', taskId)
    
    saveParseTask(taskId, filename, true)

    const poll = pollParseTaskResult(taskId, 300000)
    currentPollPromise.value = poll

    wirePollToProgress(poll, progressRef, {
      showMessage: `正在后台批量解析: ${filename}`,
      taskType: 'parse',
      completeHint: '批量解析完成',
      completeButtonText: '',
    })
    ...
    const finalResult = await poll
    console.log('[Jobs.vue] 批量解析完成:', finalResult)
    ...
    if (finalResult.is_batch && finalResult.jobs) {
      ...
      batchJobs.value = []
      ElMessage.warning('未能解析出批量职位，请检查文件内容')
    }
    
    onSuccess({})
    
  } catch (err) {
    console.error('[Jobs.vue] 批量解析失败:', err)
    clearSavedParseTask()
...
async function resumePolling() {
  ...
  if (completedResult) {
    console.log('[Jobs.vue] 检测到已完成待回填结果:', completedResult)
    ...
    try {
      ...
      // 用户点了"暂不需要"就留着 localStorage，不清理
    }
    return
  }
  ...
  console.log('[Admin Jobs.vue] 发现未完成任务，先查状态:', savedTask.taskId)
  
  try {
    const { getParseTaskStatus } = await import('../../api/job')
    const statusResp = await getParseTaskStatus(savedTask.taskId)
    const statusData = statusResp.data?.status ? statusResp.data : statusResp
    
    console.log('[Admin Jobs.vue] 任务当前状态:', statusData)
    ...
    console.warn('[Admin Jobs.vue] 查询状态失败:', e)
  }
}
